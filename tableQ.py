...
def generateTrainingData(numSamples, fname, iterPrint = 10000,numPlayers=2,targetRound=-1):
    """
    ...
    with open(fname,'w') as outfile:
        ...
        for i in range(numSamples):
            ...
            print("sdf")
            actDicts += t.playGame(0,log=True,targetRound=targetRound)
            print("sdf")
            print actDicts
            if i % iterPrint == 0:
                print(i)
...
class Player():
    def __init__(self,hand=[],nn=None,nnNext=None,dumb=True,callUntilRd=-1,varDict={},varDictNext={}):
        self.hand = hand
        self.nn = nn
        self.nnNext = nnNext
        self.dumb=dumb
        self.callUntilRd = callUntilRd
        self.varDict = varDict
        self.varDictNext = varDictNext

    def act(self,dealerIndex,playerIndex,tableCards,pot,
                bets,remainingPlayers,rd_num,evaluator,allIn,useNextNN=False):
        if self.nn == None or rd_num <= self.callUntilRd:
            return self.callAct(dealerIndex,playerIndex,tableCards,pot,
                bets,remainingPlayers,rd_num,evaluator,allIn)
        else:
            return self.nnAct(dealerIndex,playerIndex,tableCards,pot,
                bets,remainingPlayers,rd_num,evaluator,allIn,useNextNN=useNextNN)

    def genFeatDict(self,dealerIndex,playerIndex,tableCards,pot,
                    bets,remainingPlayers,rd_num,evaluator,allIn):
        featDict = {}
        featDict["dealerIndex"] = dealerIndex
        featDict["playerIndex"] = playerIndex
        unBitHandCards = []
        for card in self.hand:
            ...
            if suit == 4:
                ...
                suit = 4
            unBitHandCards.append(rank+(suit-1)*13)
        unBitTableCards = []
        for card in tableCards:
            ...
            if suit == 4:
                ...
                suit = 4
            unBitTableCards.append(rank+(suit-1)*13)
        featDict["handCards"] = unBitHandCards
        featDict["tableCards"] = unBitTableCards
        featDict["pot"] = pot
        featDict["bets"] = bets
        featDict["remPlayers"] = remainingPlayers
        featDict["rd_num"] = rd_num
        if len(tableCards + self.hand) >= 5:
            ...
            featDict["handScore"] = 10*float(playerScore)/7462
            featDict["handRank"] = handRank
        if len(tableCards) >= 5:
            tableScore = evaluator.evaluate([],tableCards)
            # TODOJUMPPOINT
            tableRank = evaluator.get_rank_class(tableScore)
            featDict["scoreDiff"] = 10*float(tableScore-playerScore)/tableScore
            featDict["rankDiff"] = tableRank-handRank
        return featDict

    def parseFeatDict(self, featDict,cardsOn=True,otherOn=False):
        sample = featDict
    ...
    def callAct(self,dealerIndex,playerIndex,tableCards,pot,
                bets,remainingPlayers,rd_num,evaluator,allIn):
        """
        Always calls (or goes all-in if the Stack is reached).
        Reponses:
            0: Fold
            1: Call
            2: Bet pot
        ...
        actAllIn = False
        if callBet+currBet > STACK:
            callBet = STACK
            actAllIn = True
        actDict = self.genFeatDict(dealerIndex,playerIndex,tableCards,pot,
                    bets,remainingPlayers,rd_num,evaluator,allIn)
        if actAllIn:
            action = (ALLIN_VAL,callBet)
            actDict["action"] = action
            return (ALLIN_VAL,callBet,actDict)
        else:
            action = (BET_VAL,callBet)
            actDict["action"] = action
            return (BET_VAL,callBet,actDict)

    def nnAct(self,dealerIndex,playerIndex,tableCards,pot,
                bets,remainingPlayers,rd_num,evaluator,allIn,useNextNN=False):
        maxBet = 0
        for bet in bets:
            if bet > maxBet:
        ...
        featDict = self.genFeatDict(dealerIndex,playerIndex,tableCards,pot,
                    bets,remainingPlayers,rd_num,evaluator,allIn)
        if self.dumb:
            parsedFeats = np.array([self.parseFeatDict(featDict,cardsOn=True,otherOn=False)])
            resultFiller = np.array([[-1]*2])
        else:
            parsedFeats = np.array([self.parseFeatDict(featDict,cardsOn=True,otherOn=True)])
            resultFiller = np.array([[-1]*4])

        # Run neural net to get action
        if useNextNN:
            actVal = self.nnNext.run(self.varDictNext["y_predict"],feed_dict={
                self.varDictNext["x"] : parsedFeats,
                self.varDictNext["y_"] : resultFiller,
                self.varDictNext["keep_prob"] : 1.0
                })
        else:
            actVal = self.nn.run(self.varDict["y_predict"],feed_dict={
                self.varDict["x"] : parsedFeats,
                self.varDict["y_"] : resultFiller,
                self.varDict["keep_prob"] : 1.0
                })
        if actVal == CALL_VAL and self.dumb:
            actVal = BET_VAL

        bet = 0
        if actVal == BET_VAL or actVal == ALLIN_VAL:
            if allIn:
                actVal = CALL_VAL
        if actVal == CALL_VAL:
            bet = callBet
            ...
            bet = potBet
        elif actVal == ALLIN_VAL:
            bet = STACK - currBet
        if bet + currBet > STACK:
            bet = STACK - currBet
            actVal = ALLIN_VAL
        action = (actVal,bet)
        featDict["action"] = action
        return (actVal, bet, featDict)
...
class Table():
    ...
    def playGame(self,dealerIndex=-1,log=False,targetRound=-1):
        self.resetTable()
        self.log = log
        self.targetRound = targetRound
        self.dealerIndex = dealerIndex
        print("table:playgame")
        # for i in range(4):
        #     self.playRound(i)
        #     print("table:playRund")
        #     print(self.logs)
        #     self.doBets(i)
        #     print("table:doBets")
        # winplayers = self.findWinners()

        self.preFlop()
        self.flop()
        self.turn()
        winplayers = self.river()
        print(self.logs)
        if self.log:
    ...
    def genTableDict(self,rd_num):
        tableDict = {}
        tableDict["dealerIndex"] = self.dealerIndex
        tableDict["pot"] = self.pot
        tableDict["bets"] = self.bets
        tableDict["remainingPlayers"] = self.remainingPlayers
        allPlayerCards = []
        for i in range(self.numPlayers):
            allPlayerCards.append(self.players[i].hand)
        tableDict["allPlayerCards"] = allPlayerCards
        tableDict["tableCards"] = self.tableCards
        tableDict["checks"] = self.checks
        tableDict["cardsToDeal"] = self.cardsToDeal
        tableDict["dealIndex"] = self.dealIndex
        tableDict["rd_num"] = rd_num
        tableDict["allIn"] = self.allIn
        return tableDict

    def setTable(self,tableDict,players):
        self.players = players
        self.dealerIndex = tableDict["dealerIndex"]
        self.pot = tableDict["pot"]
        self.bets = tableDict["bets"]
        self.remainingPlayers = tableDict["remainingPlayers"]
        self.tableCards = tableDict["tableCards"]
        self.checks = tableDict["checks"]
        self.cardsToDeal = tableDict["cardsToDeal"]
        self.allIn = tableDict["allIn"]
        self.dealIndex = tableDict["dealIndex"]
        return tableDict["rd_num"]

    def simulateResponse(self,i,response,bet):
        """ Returns True if the response triggers ALL-IN mode. """
        if response == FOLD_VAL:
            ...
            self.bets[i] += bet
            self.pot += bet
        elif response == ALLIN_VAL:
            self.checks[i] = False
            self.bets[i] += bet
            self.pot += bet
            self.allIn = True

    def simulateStep(self,tableDict,players,playerIndex,response,bet):
        """
        Returns:
        isEndOfGame,featDict,winnerList
        """
        rd_num = self.setTable(tableDict,players)
        tableCards = tableDict["tableCards"]
        self.simulateResponse(playerIndex,response,bet)

        # Finish current round
        featDict = self.doBets(rd_num,stopIndex = playerIndex,useNextNN=False)
        if featDict != None:
            return False,featDict,None
        # Simulate next round if necessary
        rd_num += 1
        if rd_num <= 3:
            self.playRound(rd_num)
            featDict = self.doBets(rd_num,startNormally=True,stopIndex = playerIndex,useNextNN=True)
            return False,featDict,None
        else:
            return True,None,self.findWinners()

    ### JUMPPOINT
    def doBets(self,rd_num,startNormally=False,stopIndex=None,useNextNN=False):
        dealerIndex = self.dealerIndex
        log = self.log
        if stopIndex == None or startNormally:
            i = dealerIndex if rd_num == 0 else dealerIndex+1
            i = i % self.numPlayers
            self.checks = [False]*self.numPlayers
        else:
            # Continue round
            i = (stopIndex+1)%self.numPlayers

        while not self.allChecked():
            if stopIndex != None and i == stopIndex:
                featDict = self.players[i].genFeatDict(
                    dealerIndex,i,self.tableCards,self.pot,
                    self.bets,self.remainingPlayers,rd_num,self.evaluator,self.allIn)
                return featDict
            if not self.remainingPlayers[i]:
                self.checks[i] = True
                i = (i+1) % self.numPlayers
                continue
            response,bet,actDict = self.players[i].act(
                dealerIndex,i,self.tableCards,self.pot,
                self.bets,self.remainingPlayers,rd_num,self.evaluator,self.allIn,useNextNN=useNextNN)
            if log and (self.targetRound == -1 or rd_num == self.targetRound):
                self.logs.append(actDict)
            self.simulateResponse(i,response,bet)
            i = (i+1) % self.numPlayers
        return None
